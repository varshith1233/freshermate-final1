...
function MaterialSelect({ onFetchData }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const queryString = `degree=${degree}&branch=${branch}&semester=${semester}&curriculum=${curriculum}`;
    ...
    try {
      ...
      console.error('Error fetching material:', error); // Handle errors
      window.alert('Error fetching material. Please try again.');
    }
  };

  return (
    <div className="container mt-4">
      <h1 className="mb-4">Get Material</h1> 
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" className="btn btn-primary">Get Material</button>
      </form>
    </div>
  );
}
